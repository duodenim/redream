...
EMITTER(LOAD_GUEST, CONSTRAINTS(NONE, IMM_I32)) {
  ...
  if (ir_is_constant(addr)) {
    ...
    if (ptr) {
      /* FIXME couldn't we directly load from guestmem + addr */
      e.mov(e.eax, addr->i32);
      x64_backend_load_mem(backend, RES, e.rax + guestmem);
    } else {
...
EMITTER(STORE_GUEST, CONSTRAINTS(NONE, IMM_I32, NONE)) {
  ...
  if (ir_is_constant(addr)) {
    ...
    if (ptr) {
      /* FIXME couldn't we directly load from guestmem + addr */
      e.mov(e.eax, addr->i32);
      x64_backend_store_mem(backend, e.rax + guestmem, data);
    } else {
...
EMITTER(STORE_FAST, CONSTRAINTS(NONE, NONE, NONE)) {
  ...
  /* FIXME support IMM addr / data */

  x64_backend_store_mem(backend, dst.cvt64() + guestmem, data);
}
